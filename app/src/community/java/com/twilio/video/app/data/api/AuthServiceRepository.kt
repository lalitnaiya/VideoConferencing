...
//        getPasscode(passcode)?.let { passcode ->
        ...
        Log.e("TAG","IDENTITY" + identity +"------" +roomName)

        val (requestBody, url) = buildRequest("12345678901092392342347927349792", identity, roomName)

        try {
            authService.getToken(url, identity!!, roomName!!).let { response ->
...
//        }
        return Pair(requestBody, "https://kids.saregama.com/create_token.php")
    }

    private fun handleResponse(response: AuthServiceResponseDTO): String? {
        return response.roomDetails.let { token ->
